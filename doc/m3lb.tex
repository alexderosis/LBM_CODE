...
Read the BGK column against the last one first, because that is the like-for-like
comparison and it is favourable in four cases out of six. The two it loses ---
the circular interface in a shear flow and the swirling sphere --- are the two
that stretch the interface into a filament a few cells across before returning
it, so they are the cases where the reconstruction, rather than the transport,
is being measured. That the same code wins on translation and on the Zalesak
disk and loses on the two filament tests is a coherent pattern rather than
scatter, and it is worth stating as a limitation rather than averaging away.
