...
float and roll under the flow's own reaction. Every scheme documented here is
...
accuracy: the reduced lattice moves 5 or 7 populations against 9 or 19 and pays
...
and one store more, not a second pass. Only the potential-form fluid operator
reads it.
...
Two fluid operators exist. \code{MultiphaseBGK} is the matched-density one: $\rho$
is a single field near 1, and $\phi$ enters the flow only through the capillary
stress of \S\ref{sec:surftens} and, optionally, through a per-node relaxation
rate (\code{omega\_n}) for a viscosity contrast. \code{MultiphasePotentialBGK}
carries a density ratio, and to do so it changes what the zeroth moment means.
...
that is a one-sided derivative taken as though it were central. Every case that
currently runs this operator is periodic. The gap has the same shape as the
missing wetting condition, and the same remedy would be needed.
...
product lattice: D2Q9 or D3Q27, enforced by a static assertion, since D3Q19 would
need the monomial basis and its own equilibrium moments. And it takes raw storage
...
the driver rather than physics. The scheme is two-dimensional, and says so in the
compiler's voice --- a \code{static\_assert} on $D=2$, because on a 3D lattice it
would silently model an infinite prism in $z$ with a rigid-body solve carrying one
angle where it needs three. Three dimensions is a $6\times6$ with a rotating
inertia tensor and a quaternion, a different piece of work; the fluid side would
not change at all. And the body is of uniform density: its centre of mass is taken
to be the centre of the rectangle, which is exactly what lets its own weight drop
out of the roll equation. Ballast low in the hull is the standard way to make a
tall body float upright, and it is precisely what this cannot express --- it would
need a second centre, a $\overline{BG}$ that no longer follows from the draft, and
a weight torque restored to the third row. The rectangle is also the only
indicator provided, though anything carrying a signed distance function drops
straight in.
...
equation. The colour-gradient model of Saito \emph{et al.}\ (Phys.\ Rev.\ E
\textbf{98}, 013305, 2018), implemented in \code{ColourGradient.hpp} and
...
\paragraph{A factor of two that a running simulation would absorb.} The
perturbation is written per colour in Eq.~(30) as
$(A_k/2)|\nabla\phi|[w_i(\ci\cdot\nn)^2 - B_i]$, and Eq.~(39) sums the capillary
stress over $k$ as well as over $i$ with $A = A_r = A_b$. Applied to the
colour-blind population --- which is what the paper's own text says is done ---
the two halves add and the coefficient is $A$, not $A/2$. The distinction is not
cosmetic, and it was settled by deriving $\sigma$ from Eq.~(39) rather than
copying Eq.~(32): for a flat interface
\begin{equation}
  \sigma = \int (S_{tt} - S_{nn})\,\mathrm{d}n
         = \frac{2A\tau}{9}\,\Delta\phi
         = \frac{4A\tau}{9},
\end{equation}
since $\phi$ runs from $-1$ to $+1$. That is Eq.~(32) exactly. With $A/2$ it
would have come out half as large, and a static droplet would have reported a
surface tension $50\%$ low with every other property of the model intact.
...
\midrule
phase field (\S\ref{sec:ace}) & diffuse & both & $\sim 100$, with the gauge problem \\
...
standard way of making a tall body float upright that this cannot express. And
any of it in three dimensions, where the rigid-body solve would be a $6\times6$
with a rotating inertia tensor and a quaternion; the fluid side would not change.

%------------------------------------------------------------------------------
\subsubsection{A published multiphase campaign: De Rosis and Enan (2021)}
\label{sec:enan}

De Rosis and Enan \cite{derosis2021} is the source paper for this tree's phase-field
formulation (\S\ref{sec:multiphase}), and its Sec.~III is nine test cases with
tabulated numbers. That makes it the same kind of target as
\S\ref{sec:campaign}: not one benchmark but a campaign, where the interesting
result is the pattern across the cases rather than any single row.

The nine split into three groups, and only the first two are run here.

\begin{itemize}
  \item \textbf{Interface capture} (their III.A--III.F, Tables I--VII). The
    velocity field is \emph{prescribed}; only the order parameter is solved.
    No fluid collision operator is exercised at all, so these isolate the
    Allen--Cahn discretisation from everything else.
  \item \textbf{Rayleigh--Taylor instability} (their III.H--III.I,
    Tables VIII--X). Fully coupled, and the only quantitative check is the
    trajectory of the spike.
  \item \textbf{Vertical penetration of a wedge} (their III.G). Not run --- see
    the end of this section for why, which is a missing capability rather than
    a missing run.
\end{itemize}

\paragraph{What is compared, and to which column.} The paper computes
$\bm{\nabla}\phi$ two ways --- finite differences, its Eq.~(20), and directly
from the first-order central moments, its Eq.~(70) --- and tabulates both. This
tree implements the finite-difference route only, and the reason is structural:
Eq.~(70) reads the phase field's own populations at the node, and under Esoteric
Pull the slots a node reads are the slots it is about to write
(\S\ref{sec:esopull}), so the gradient would be taken from a mixture of
pre- and post-collision states. Every comparison below is therefore against the
paper's FD column.

Two further differences are deliberate and are stated here once rather than
repeated per case. The paper's phase field is D3Q19 with a written-out
$19\times19$ transform; this tree runs D3Q27, because the central-moment
transform here is three one-dimensional passes and needs a product lattice
(\S\ref{sec:eqprod}). And a two-dimensional case is run as D3Q27 with a
single periodic cell in $z$, which is exact rather than approximate: D3Q27 is a
product lattice, so at $n_z=1$ it reduces to D2Q9 identically.

\paragraph{The six interface-capture cases.} Table~\ref{tab:enan} collects them.
It is generated from \code{results/L\_enan/enan\_interface.dat} by
\code{doc/fig/enan\_interface\_table.py} rather than transcribed, so the
document cannot drift from the data behind it.

\begin{table}[htbp]
  \centering
  \caption{De Rosis and Enan's Sec.~III interface-capture cases, D3Q27, FP64,
  compared against their finite-difference column. The velocity is prescribed
  in all of them. \textit{diverged} is a run that reached a non-finite state,
  not one that finished badly.}
  \label{tab:enan}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig/enan_phase_field.png}
  \caption{The interface-capture campaign. (a) the six cases of their
  III.A--III.F, both collisions against their finite-difference column; note
  that the translation case is the one where the central-moment operator loses
  badly and the Zalesak disk the one where it wins. (b) their Tables~I and III;
  a cross marks a run that reached $10^{300}$, and the three of them are the
  reason the central-moment operator exists. (c) the cycle scan, which is the
  measurement that disproved a crossover in $\omega_\phi$: BGK's translation
  error is flat across a tenfold change in run length and the central-moment
  error compounds. (d) the mobility ladder at fixed run length, which is what
  made the $\omega_\phi$ reading look right in the first place.}
  \label{fig:enanphase}
\end{figure}

Read the BGK column against the last one first, because that is the
like-for-like comparison. It is favourable on four of the seven rows where both
complete --- the Geier translation, the Zalesak disk, and both smoothed-shear
cases --- and unfavourable on three, and the three are worth separating because
they fail for two different reasons.

The circular interface in a shear flow and the swirling sphere are the two that
stretch the interface into a filament a few cells across before returning it,
so they measure the reconstruction rather than the transport; being 14\% and
31\% off there is a limitation of the scheme on thin filaments and is stated as
one rather than averaged away.

Their Table~I is a different and worse story, and it is the one honest failure
in the section. Both operators complete all three Peclet numbers --- BGK gives
0.0781, 0.0267 and 0.0496 against their 0.0066, 0.0050 and 0.0090, and the
central-moment collision 0.3053, 0.1190 and 0.0731 --- so nothing diverges and
nothing is missing; the numbers are simply five to forty-six times the
published ones. That \emph{both} operators miss by so much is what makes this
informative: it locates the deficit in what they share --- the gradient
stencil, the anti-diffusion source, the initialisation --- rather than in
either collision, and it is recorded unresolved.

Two details sharpen it rather than explain it away. The same physical test at
half the run length and half the Peclet number, their Table~II, has BGK
\emph{beating} the published column at 0.0082 against 0.0134, so whatever this
is switches on between the two. And the error is not monotonic in $Pe$ for
either operator, which rules out a simple under-resolution of the interface at
low mobility.

\paragraph{The central-moment collision does not dominate BGK, and what it
buys is the stability ceiling rather than accuracy.} This is the least expected
result in the section, and two explanations were attempted for it here before
one survived contact with the data. Both failures are recorded because each was
a plausible reading of a real measurement.

The first was a crossover in $\omega_\phi$. The mobility ladder below is a
genuine measurement and it looks exactly like one --- until the Zalesak disk is
run at $\omega_\phi = 1.991$ against translation's 1.988 and the ordering
reverses. The second was run length, on the strength of the cycle scan below,
which is also a genuine measurement; that one survived until the spherical
interface came in 1.48 times worse at 5000 steps while the Zalesak disk was
0.46 times at 10\,000. Table~\ref{tab:enanratio} is the evidence that no single
variable orders the two operators.

\begin{table}[htbp]
  \centering
  \caption{The two collisions on the cases where both complete, ordered by run
  length. Neither run length nor $\omega_\phi$ orders the last column.}
  \label{tab:enanratio}
  \input{fig/enan_interface_ratio.tex}
\end{table}

What \emph{is} established, and is the reason the operator exists, is the
stability ceiling. The mobility fixes the relaxation rate,
$M = c_s^2(1/\omega_\phi - \tfrac{1}{2})$, and the Peclet number fixes the
mobility, $Pe = U_0\xi/M$; so a high Peclet number \emph{is} an $\omega_\phi$
approaching 2, with nothing to trade against it. BGK relaxes every moment at
that rate, ghost modes included, and dies. This operator relaxes only the first
three. Their Table~III is therefore completed here at all four published
Peclet numbers --- 80, 400, 800 and 4000 --- with errors of 0.0131, 0.0237,
0.0284 and 0.0336 against their 0.0593, 0.0590, 0.0558 and 0.0505, where BGK
manages $Pe=80$ alone and reaches $10^{300}$ on the other three. Every one of
those rows is better than the published column, and the drift in $\sum\phi$
stays at $3\e{-13}$ across all of them.

\paragraph{Two measurements that stand on their own.} The first is the cycle
scan, on their Table~II translation with only the number of cycles varied and
$\omega_\phi$ held --- one cycle is 5000 steps:
...
\begin{tabular}{rrrr}
  ...
  Cycles & Steps & BGK & CM \\
  \midrule
  1  & 5\,000  & 0.00814 & 0.01593 \\
  2  & 10\,000 & 0.00803 & 0.01671 \\
  5  & 25\,000 & 0.00804 & 0.02276 \\
  10 & 50\,000 & 0.00817 & 0.03968 \\
  \bottomrule
\end{tabular}
\end{center}

BGK is flat to within 2\% across a tenfold change in run length: on this case
its error saturates within the first cycle and stays there. The central-moment
error compounds, and there are two separable components --- it starts twice as
bad and it also grows. That is a property of \emph{this} case, sustained uniform
advection, and the ratio table above is why it is not offered as the general
explanation.

The second is the mobility ladder, at fixed ten cycles:
...
BGK's saturated error is large at low $\omega_\phi$ and small near 2. Note that
$Pe=60$, the paper's own operating point, is the last row: on that case as
published, on this lattice, the central-moment collision is the worse of the two.

Neither operator is at fault, and the reason it cannot be either of them is that
both put identical zeroth and first moments into the post-collision state.
\code{tests/test\_phase\_field.cpp} checks that directly and it holds to
$3\e{-18}$, which is round-off; every moment entering the Allen--Cahn equation
at leading order is therefore shared, and the entire difference between them is
the ghost modes --- which BGK relaxes at $\omega_\phi$ and the central-moment
operator sends to equilibrium.

One thing is left unexplained rather than resolved: on the translation case the
central-moment collision here is also three times worse than the paper's own
D3Q19 central-moment result of 0.0134 at the same Peclet number. The setup was
checked against the source for the five things that could have made that
artificial --- the initial profile is their Eq.~(72), the gradient is their
isotropic Eq.~(20) stencil, the error is their Eq.~(71), and $L_0$, $R$, $\xi$,
$U_0$ and the cycle count all match --- so it is a real disagreement, and it is
recorded as one.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig/enan_rt_wedge.png}
  \caption{Rayleigh--Taylor and wedge entry. (a) the 2-D spike against their
  Table~VIII at three Reynolds numbers --- the pass. (b) the two 3-D cases
  against their Tables~IX and X, with the band their Table~IX collects from
  eight published models; being outside it from $t/t_0=1$ is the failure, drawn
  rather than asserted. (c) wedge entry at three deadrise angles with Wagner's
  line for each and the fit window shaded; the $30^\circ$ curve collapsing
  inside the window has the same signature as (d) and is the same cause. (d)
  the release height: identical impact and window, and at 20 cells the pulse
  from the impulsive release returns inside the window and takes the fitted
  slope from $29.55$ to $6.51$.}
  \label{fig:enanrtwedge}
\end{figure}

\paragraph{Rayleigh--Taylor instability.} Their III.H and III.I are the first
cases in this group where the fluid is solved at all: the phase field is coupled
to \code{MultiphaseCentralMoments} through the interfacial force, and both
lattices run central moments. Four configurations are tabulated, and the paper's
own list of cases announces only three of them --- its Table~X,
three-dimensional at $Re=30\,000$, appears without being introduced, and is run
here because it is there.

The measured quantity is the vertical position of the spike normalised by the
domain width, at $t/t_0 = 0,\,0.5,\dots,3$ with $t_0=\sqrt{W/(g\,\mathrm{At})}$.
Table~\ref{tab:enanrt} is generated from \code{results/L\_enan/enan\_rt.dat}.

\begin{table}[htbp]
  \centering
  \caption{Rayleigh--Taylor spike position, D3Q27 on both lattices, against
  their Tables VIII, IX and X. Their values in italics. Table~X ends at exactly
  zero, so that instant is excluded from the worst relative deviation rather
  than reported as infinite.}
  \label{tab:enanrt}
  \input{fig/enan_rt_table.tex}
\end{table}

\subparagraph{Two dimensions: a pass, and a resolution requirement.} The 2-D
case agrees with their Table~VIII to 2.7\% at worst and to under 1\% through
most of the run. That is the whole of III.H, and it is the strongest evidence in
this section that the coupled multiphase solver is right, because unlike the
interface-capture group it exercises the fluid operator, the interfacial force
and the coupling order together.

It also comes with a resolution requirement that is worth stating separately,
because it is the first thing to check before reading anything into a
disagreement:

\begin{table}[htbp]
  \centering
  \caption{The same case, 2-D at $Re=30\,000$, with only $W$ and $Ca$ varied.
  The deficit is entirely late: the first five instants agree to about 1\% at
  every resolution.}
  \label{tab:enanrtres}
  \input{fig/enan_rt_resolution.tex}
\end{table}

The scheme converges --- 15.8\%, 4.9\%, 2.7\% at $W=64$, 128, 256 --- and the
error is late in every case, appearing only once the spike has rolled up and
what has to be resolved is the filament rather than the interface.

The $Ca$ rows correct something this document got wrong on first writing. At
$W=64$, raising $Ca$ from 0.26 to 960 --- dropping $\sigma$ from
$1.58\e{-4}$ to $1.25\e{-6}$ --- takes the worst deviation from 15.8\% to
29.4\%, which reads exactly like surface tension adding a second, independent
penalty. It does not. The same change at $W=256$ gives 1.9\% against 2.7\%,
very slightly \emph{better}. A nearly surface-tension-free interface simply
needs more cells to carry the filament; given them, it costs nothing.

\subparagraph{Three dimensions: a failure, reported as one.} Both 3-D cases fall
much faster than the published tables --- 92.8\% and 46.2\% at worst, with the
$Re=256$ spike reaching 0.047 where all eight models their Table~IX collects lie
between 0.648 and 0.863. This is not presented as agreement inside a spread; it
is outside it from $t/t_0=1$ onward.

What has been ruled out is worth as much as what has not. The setup is theirs:
their Eq.~(83) puts a domain of $W\times4W\times W$ at $W=64$, the interface at
$2W$ and a perturbation $0.05W[\cos2\pi x + \cos2\pi z]$, and all three match.
The \emph{measurement} is also not at fault, which was the leading hypothesis
here and a reasonable one: ``position of the spike'' means the tip of the
coherent finger, whereas a global minimum over $\phi>\tfrac12$ becomes a
different quantity the moment anything detaches, and $Ca=960$ is the regime an
interface fragments in. \code{validation/enan\_rt} therefore reports three spike
measures side by side --- the global minimum, the tip on the axis where
Eq.~(83) puts the finger, and one thresholded on a cross-section population so
a single stray cell cannot set it. On the 3-D $Re=256$ case \emph{they agree to
the digit at every one of the seven instants}, 0.047 included. There is no
detached material: the finger really does reach the floor.

So the deficit is physics, and it is only partly accounted for. Their 3-D cases
run at $W=64$, where the 2-D ladder above already costs 15.8\%, and at
$Ca=960$, which costs nothing on its own. That leaves most of the gap
unexplained, and the run that would test whether the residue is also resolution
--- three-dimensional at $W=96$ or beyond --- costs several hours and several
gigabytes and has not been done. It is recorded here as an open disagreement
with the bound that is known, rather than as a limitation with a cause.

The two remaining 2-D configurations, $Re=256$ and $Re=3000$, have no table in
the paper; its Fig.~11 plots them against four other studies. They are run and
reported as trajectories rather than scored, and behave as viscosity should:
the spike at $t/t_0=3$ sits at 1.324, 0.668 and 0.570 for $Re=256$, 3000 and
$30\,000$.
...
deadrise angle, and the case needs no conversion out of lattice units despite
the paper stating it in metres and seconds.
...
The one thing that costs is surface accuracy, and it is worth being precise
about where. The product of two $\tanh$ indicators rounds a convex corner, and
...
\subparagraph{Results.} Table~\ref{tab:enanwedge} is the deadrise sweep at
$b=80$.

\begin{table}[htbp]
  \centering
  \caption{Wedge entry against Wagner. The slope is fitted over the middle of
  Wagner's own validity window, $0.25 < \tau/\tau_{\rm valid} < 0.70$ with
  $\tau_{\rm valid} = 2\tan\varphi/\pi$; below that the wetted length is a few
  cells across, above it the spray root nears the knuckle.}
  \label{tab:enanwedge}
  \begin{tabular}{rrrrrl}
    \toprule
    $\varphi$ & Slope & Wagner & Dev. & $R^2$ & \\
    \midrule
    $10^\circ$ & 107.15 & 124.66 & $-14.0$\% & 0.991 & \\
    $15^\circ$ &  60.64 &  53.98 & $+12.3$\% & 0.999 & \\
    $20^\circ$ &  29.55 &  29.26 & $+1.0$\%  & 0.997 & \\
    $30^\circ$ &   7.69 &  11.63 & $-33.9$\% & 0.106 & \textit{see below} \\
    $45^\circ$ &   3.15 &   3.88 & $-18.6$\% & 0.208 & \textit{reflections} \\
    \bottomrule
  \end{tabular}
\end{table}

At $10$, $15$ and $20$ degrees the force is linear in time to $R^2 \ge 0.99$
and the slope is within 14\%, 12\% and 1\% of Wagner. That is the regime Wagner
is asymptotically valid in --- his result is derived for vanishing $\varphi$ ---
and the paper reports the same trend of improving agreement as the angle falls.

The two largest angles failed on the first pass, and the cause was one thing
rather than two. Wagner's window $\tau_{\rm valid} = 2\tan\varphi/\pi$
\emph{grows with the deadrise angle}, so at $45^\circ$ the fit runs to 1783
steps after touchdown while an acoustic wave crosses the periodic domain in
1109: the fit was being taken through the simulation's own reflections. At
$30^\circ$ the step-count bound said the window closed just inside that, and
the force appeared instead to saturate --- which was written up here as
unexplained until Fig.~\ref{fig:enanrtwedge}(c) was drawn and showed the
$30^\circ$ curve collapsing to near zero inside the window, with exactly the
signature of panel (d). It was the same artefact, and looking at the curve was
what settled it; a step-count estimate had said otherwise.

The remedy is to size the domain by the acoustic time rather than by the body,
which is what \code{validation/enan\_wedge} now does: it takes
$n_x = \max(8b,\,1.6\,c_s \times 0.7\,\tau_{\rm valid}\,b/v)$, leaving
$10^\circ$ and $20^\circ$ untouched at $8b$ and widening $30^\circ$ to 950
cells and $45^\circ$ to 1646. A fixed multiple of the body cannot work here,
because the requirement scales with $\tan\varphi$ and the body does not.

\subparagraph{Two traps, one of which nearly produced a wrong conclusion.} The
first is the gauge: the multiphase populations carry a normalised pressure, not
a density, so seeding \code{FlowState} with $\rho$ starts a run that is finite,
plausible, and dead fifteen steps later with nothing to indicate why.

The second is worth more space. Released with its apex on the surface, the
wedge delivers an added-mass impulse that has nothing to do with slamming:
$F^*=35.8$ at contact against a Wagner value of zero there and about 7 at the
end of the window, and the force then \emph{decays} through the window instead
of growing. Releasing it above the surface fixes the trend --- but the drop
must also outlast the acoustic pulse the impulsive release launches, and that
is a second requirement. Released 20 cells up at $b=80$, the force rises
cleanly to $F^*=5.84$ and then collapses to 1.29 and recovers, in the middle of
the fit window. It looks exactly like the spray root doing something physical.
Changing only the drop settles it:
...
\begin{tabular}{lrrr}
  ...
  Drop & Slope & Dev. & $R^2$ \\
  \midrule
  20 cells & 6.51  & $-77.8$\% & 0.034 \\
  60 cells & 29.55 & $+1.0$\%  & 0.997 \\
  \bottomrule
\end{tabular}
\end{center}

Same impact, same window, same everything else. The natural next move would
have been a resolution study, and it would have been actively misleading:
$b=40$ scores \emph{better} than $b=80$ at the short drop, because a smaller
domain moves the artefact rather than removing it. The default is now $0.75b$,
which buys about 2.7 acoustic crossings at every $b$ --- a fixed cell count
would have hidden the same bug at a different size.

\subparagraph{The density ratio, which turned out not to be the obstacle.}
Water over air is $1000/1.225 = 816$, and the phase field here is characterised
to about 100, so the expectation was that the paper's own fluids would be out
of reach and a reduced ratio would have to be reported instead. Measured at
$\varphi=20^\circ$:
...
\begin{tabular}{rrrr}
  ...
  Ratio & Slope & Dev. & $R^2$ \\
  \midrule
  10  & 103.29 & $+253.1$\% & 0.986 \\
  50  &  30.06 & $+2.8$\%   & 0.987 \\
  100 &  29.55 & $+1.0$\%   & 0.997 \\
  200 &  31.14 & $+6.4$\%   & 0.999 \\
  816 &  32.73 & $+11.9$\%  & 0.9999 \\
  \bottomrule
\end{tabular}
\end{center}

The paper's 816 runs, with the best linearity of the sweep, so this case does
not depend on the reduced-ratio approximation at all; only a ratio of 10 fails,
and it fails loudly. The deviation grows mildly with the ratio while Wagner ---
who has no air whatever --- ought to be approached as the air thins, so two
effects are working against each other here and this sweep does not separate
them.
...
this. It runs at \textbf{354.6}~MLUPS with \textbf{0 bytes} of stack frame on
every one of its six kernels, by construction rather than after a measurement.
...
\paragraph{An instability that was the viscosity, not the model.} At a density
ratio of $100$ the phase-field droplet diverged, and it was nearly written up as
a capability limit. Matching the \emph{dynamic} viscosity across the ratio
leaves the heavy phase with a hundredfold smaller kinematic viscosity, so
...
current twenty times smaller. The driver now prints $\nu$ and $\omega$ per phase
before the run and flags $\omega > 1.9$, because that number was reconstructed
in a post-mortem when it could have been on screen beforehand.
...
\item \textbf{No free surface}, and the reason is streaming rather than effort:
  \S\ref{sec:fstwolattice}. Porting it means a two-lattice storage path plus
  five kernel launches with a fence between each.
\item \textbf{No central moments for the phase field's fluid operator.} The
  parent has \code{MultiphaseCentralMoments.hpp}; what is on the GPU is the BGK
  potential form. The colour gradient \emph{is} in central moments.
\item \textbf{No wetting, no contact angle, no external force field, and no
  open boundary for $\phi$.}
...
\item \textbf{The CUDA implementation of \S\ref{sec:cuda} covers a fraction of
  ...
  Both immiscible models of \S\ref{sec:multiphase} are there
  (\S\ref{sec:cudamp}); the free surface is not, and cannot be without a
  two-lattice path. It exists to answer one question and to carry the
  multiphysics forward on a device, not to replace this code.
...
\item \textbf{The phase field has no open boundary.} The scalar's donor
  ...
  and the same fence, but it is not written. Every multiphase case here is
  therefore periodic in one direction and closed by no-slip walls in the other;
  an interface that must leave the domain is not available.
...
\item \textbf{The multiphase central-moment operator does not run on D3Q19.}
  It is built on \code{ProductBasis}, and the \code{static\_assert} says so: a
  product lattice is required --- D2Q9 or D3Q27 --- and D3Q19 would need the
  monomial basis and its own equilibrium central moments. The pressure-form BGK
  operator carries no such restriction, asking only for a Navier--Stokes
  lattice, so a D3Q19 two-phase run is possible at single-relaxation-time
  quality --- which is exactly the quality that failed at $\mathrm{Re}=30000$ in
  \S\ref{sec:rt}.

\item \textbf{Shifted storage is unavailable to the multiphase operators.}
  \code{ShiftedPopulations} centres the stored variable on $\tilde p = 1$, which
  is precisely the pressure gauge that must be avoided at a density ratio, and
  it assumes a reference density of exactly $1$ --- at a ratio of $10$ the
  stored populations are no longer small and the shift stops buying anything.
  \code{MultiphaseCentralMoments} therefore declares \code{RawPopulations}, and
  the FP32 accuracy argument that shifted storage exists for does not apply to
  these cases.
...
\item \textbf{The rigid body is two-dimensional.} \code{PenalisedBody} carries
  one angle and one angular velocity about $z$, and says so in the compiler's
  voice as well as in prose: instantiated on a 3D lattice it would silently
  model an infinite prism. A three-dimensional body is a $6\times6$ system with
  a rotating inertia tensor and a quaternion, and does not exist here. The fluid
  side would not change.
