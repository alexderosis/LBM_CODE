...
\paragraph{The wedge, and why it is absent.} Their III.G drives a symmetric
wedge of deadrise angle $\varphi$ into quiescent water at a constant
$v=1\,$m\,s$^{-1}$ with a Peskin immersed boundary, and compares the resultant
against Wagner's potential-flow slamming force,
\[
  F_W(t) = \rho_H \frac{\pi^2 v^2 a(t)}{4\tan\varphi},
  \qquad a(t) = \frac{\pi v t}{2\tan\varphi},
\]
$a(t)$ being the wetted length. Three things would have to be built for that
number to mean anything here. There is no immersed boundary in this tree at all:
rigid bodies are handled by volume penalisation
(\S\ref{sec:penalisation}), which is a different discretisation of a different
boundary condition. The rigid body is two-dimensional and \emph{falls} --- the
reaction is fed back into Newton's equations --- whereas Wagner's result is for
prescribed constant-velocity entry, so the comparison would need a driven-body
mode that does not exist. And the body machinery is wired to the free-surface
solver, not to the phase field, so it is the wrong two-phase model for this
case.

\code{demonstrator/water\_entry} is the nearest thing in the tree and is
deliberately \emph{not} this test: it drops a flat-bottomed square, whose impact
pressure is singular at first contact and has no closed form, so it is
registered as a demonstrator and reports a force history whose shape is
physical rather than a number to quote. Presenting it as a reproduction of
III.G would be exactly the kind of claim this document warns about throughout.
