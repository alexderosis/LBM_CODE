...
Their Table~I is a different and worse story. At $Pe=125$ BGK gives 0.0781
against their 0.0066, a factor of twelve, and the central-moment collision is
worse again at 0.3053. This is the one place in the section where \emph{both}
operators are far from the published result, which locates the deficit in what
they share --- the gradient stencil, the anti-diffusion source, the
initialisation --- rather than in either collision. It is not resolved here.
Note that the same physical test at half the run length and half the Peclet
number, their Table~II, has BGK \emph{beating} the published column at 0.0082
against 0.0134, so whatever it is switches on somewhere between the two.
