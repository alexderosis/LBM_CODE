...
\subparagraph{Two dimensions: a pass.} The 2-D case agrees with their Table
VIII to 2.6\% at worst and to under 0.5\% through the first three quarters of
the run. That is the whole of III.H, and it is the strongest evidence in this
...
\item \textbf{The rigid body's two paths are separate, and the 6-DOF one is
  ...
  shape: \code{Rect} and \code{Wedge} declare \code{six\_dof = false} and use the
  validated $3\times3$, \code{Box} and \code{Sphere} are three-dimensional, and
  mixing a shape with the wrong solve is a compile error rather than a wrong
  answer. \code{tests/test\_rigid3d} pins the $6\times6$ against that $3\times3$
  ...
  \code{demonstrator/cube\_entry} is a demonstrator, and the honest bound on it
  is its own symmetry run. A cube released exactly corner-down has a three-fold
  symmetry axis, which cannot be aligned with D3Q27's four-fold axis about $y$,
  so the discrete torque is not zero: at $D=12$ it turns the body diagonal by
  $0.095^\circ$ over $t/t_0=6$, against a $0.44^\circ$ response to a
  deliberate $5^\circ$ tilt. The response is resolved by a factor of about
  $4.5$ and no more, so the \emph{sign} of that response is a result and its
  magnitude is not.
