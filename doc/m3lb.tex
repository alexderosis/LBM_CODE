...
\begin{figure}[htbp]
  ...
  rather than asserted. (c) and (d) are from a superseded parameterisation of the
  wedge --- deadrise $10$--$45^\circ$ and a body-reaction force, before their
  own $2$ and $3^\circ$ and their surface-pressure measure were adopted --- and
  are kept because each records a way this case misleads: in (c) the
  $30^\circ$ curve collapses inside the fit window with the same signature as
  (d), and in (d) an identical impact gives a fitted slope of $29.55$ or
  $6.51$ according to release height alone, because the pulse from an
  impulsive release returns inside the window. The reaction force is what makes
  both possible; the surface-pressure measure of Table~\ref{tab:enanwedge} is
