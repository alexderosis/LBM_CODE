...
\mathrm{m^2s^{-1}}$ both NaN on the crop, at the tower's Reynolds number where a
steady solve would be expected to fail --- that failure IS the point of using
this mode, not a defect in it. $\nu=8$ runs and never settles: its residual
oscillates, $8.2\times10^{-3}$ at $4000$ steps against $7.0\times10^{-2}$ three
hundred steps earlier, which is what a genuinely unsteady field looks like on
the same probe a converged one uses to declare itself finished.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig/urban_mcr_coupled_collage.png}
\caption{Coupled unsteady dispersion, Manchester crop, $3.7$~min into a
street-level release at $7.5$~m with $0.9$~km of fetch. The plume threads
between towers rather than following their time-averaged outline, and the
filamented plan view is the signature of the difference: the frozen-wind plan
views elsewhere in this section are smooth cones. $D=5\,\mathrm{m^2s^{-1}}$, the
value that diverged outright under the prescribed wind (\S\ref{sec:urbanstab}).
The animation is \texttt{doc/fig/mcr\_coupled\_unsteady.mp4}.}
...
settling back near it. A wind that can recirculate spreads the street-level
plume sideways \emph{faster} than the air above the roofs, rather than merely
failing to confine it, and this is the wake mechanism \S\ref{sec:urban}'s header
lists as absent from the prescribed profile: turbulent structure shed off one
building disperses material laterally in a way a straight-line wind, solved or
not, cannot produce. Row one, at $s=55$~m, is still the source's immediate
vicinity and reads like the confined case for the same reason it did in
\S\ref{sec:urbannyc} --- the plume has not yet had a wake to encounter.
...
one city and one geometry through it.
