...
\paragraph{The run is not converged, and the figures should be read accordingly.}
The viscous diffusion time across the vessel is $R^2/\nu = 20\,552$ steps, which
is $10.3$ beats; the run of $12\,800$ steps is $0.62$ diffusion times. Periodic
steady state needs several. Sampling at matched phase makes the shortfall
explicit --- both the mean and the median speed rise monotonically through all
six beats, so this is the bulk flow still developing and not an artefact of an
outlier:

\begin{center}\small
\begin{tabular}{clcccc}
\hline
beat & step & mean & median & $p_{99}$ & $|\uu|_{\max}$ \\
\hline
\multicolumn{6}{l}{\emph{systolic peak}, $\phi=0.5$}\\
2 & 3000  & 0.00395 & 0.00430 & 0.00822 & 0.03045 \\
4 & 7000  & 0.00429 & 0.00447 & 0.00895 & 0.03105 \\
6 & 11000 & 0.00457 & 0.00473 & 0.00961 & 0.03148 \\
\multicolumn{6}{l}{\emph{diastolic trough}, $\phi=0$}\\
2 & 4000  & 0.00284 & 0.00163 & 0.01253 & 0.05035 \\
4 & 8000  & 0.00314 & 0.00187 & 0.01379 & 0.05288 \\
6 & 12000 & 0.00339 & 0.00213 & 0.01456 & 0.05503 \\
\hline
\end{tabular}
\end{center}

The mass error does converge --- $3.34\e{-3} \to 3.51\e{-4}$ at matched phase,
roughly halving every two beats --- but the outlet density is still walking down
by exactly the per-application clamp $5\e{-4}$ every cycle, which is the
controller saturating rather than settling.

\paragraph{Phase lag.} Over the final beat the \emph{mean} speed peaks at
$\phi=0.60$ against a drive peaking at $\phi=0.50$: a lag of $0.10$ of a cycle,
$36^\circ$, which is the expected sense and order for $\alpha=8$. The $p_{99}$
tail behaves differently, peaking at $\phi\approx0.05$ --- nearly antiphase --- and
this is visible in Fig.~\ref{fig:aortapulse}, where at the diastolic trough the
root has gone pale while the narrow descending aorta is still carrying the
fastest flow in the vessel. A transit-time explanation does not survive
arithmetic: at these speeds a fluid particle needs several beats to cross the
vessel. The honest statement is that the fast tail and the bulk are out of phase
with each other, that the effect is reproducible across beats, and that it has
not been explained here. It should not be quoted as a Womersley result.
$|\uu|_{\max}$ is quoted only for completeness and should not be used at all: it
is a single staircase-corner voxel, with $p_{99.9}=0.026$ against $p_{100}=0.070$
in the steady run.
...
\end{tabular}
\caption{\textbf{Pulsatile inflow}, sixth beat, $\alpha=8.04$, shared colour
scale. The inset traces \eqref{eq:aortawave} with a marker at the current phase.
At systole the root carries a visible jet; at diastole it has gone pale while the
descending aorta still carries the fastest flow, which is the antiphase of the
$p_{99}$ tail discussed in \S\ref{sec:aortapulse}. The flow is still developing
at this point --- $0.62$ viscous diffusion times --- so these are not
periodic-steady fields.}
