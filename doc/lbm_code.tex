...
%------------------------------------------------------------------------------
\paragraph{A race that does not fire, and where it would.}
...
in the same pass --- is rewriting the slots the outflow node is reading. That is
a data race by construction, and whether the pre- or post-collision state is
obtained is not determined by the program.
...
spatial neighbour, so an outflow node and its donor sit close together in that
list and fall inside one thread's contiguous chunk. Only a pair straddling a
chunk boundary can race, and with eight threads there are seven boundaries
against two thousand outlet nodes.

\textbf{That guarantee is a property of contiguous CPU chunking and does not
survive a GPU.} There, every node is its own work item and all of them are
concurrent; no ordering between an outflow node and its donor is implied at all.
The aorta has never been run on a GPU, so this is untested rather than broken ---
but it is the first thing to fix when the fluid solver is ported, and the fix is
known: the scalar solver's open boundary (\S\ref{sec:scalaroutflow}) avoids the
same hazard by handling those cells in a second pass after a fence, reading the
macroscopic field rather than populations.
